...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(),'1.2.33') < 0) {
            $connection->addIndex(
                $setup->getTable('mailchimp_sync_ecommerce'),
        ...
        if (version_compare($context->getVersion(),'101.2.34') < 0) {
            $connection->addColumn(
        ...
        if (version_compare($context->getVersion(), '101.2.43') < 0) {
            $connection->addIndex(
                $setup->getTable('mailchimp_errors'),
        ...
        if (version_compare($context->getVersion(), '101.2.45') < 0) {
            $connection->addIndex(
                $setup->getTable('mailchimp_sync_ecommerce'),
        ...
        if (version_compare($context->getVersion(), '101.2.50') < 0) {
            $connection->addColumn(
                $setup->getTable('mailchimp_errors'),
                'added_at',
                [
        ...
        if (version_compare($context->getVersion(), '101.2.51') < 0) {
            $connection->addColumn(
                $setup->getTable('mailchimp_sync_batches'),
        ...
        if (version_compare($context->getVersion(), '101.2.66') < 0) {
            $table = $connection
